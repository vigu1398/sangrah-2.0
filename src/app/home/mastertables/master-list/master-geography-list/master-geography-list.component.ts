...
@Component({
  selector: 'app-master-geography-list',
  templateUrl: './master-geography-list.component.html',
  styleUrls: ['./master-geography-list.component.scss']
})
export class MasterGeographyListComponent implements OnInit {
  public selectedItem;
  ...
  getData(item, i: number) {
    if (this.isSelected) {
      ...
      this.addedItems = [];
      /* if (item._id) {
        this.masterService.getGeoRecord(item._id).subscribe((response) => {
  ...
  save() {
    if (this.list) {
      ...
      this.masterService.updateMasterGeo(this.list).subscribe((res: any) => {
  ...
  delete() {
    ...
    dialogRef.afterClosed().subscribe((result: any) => {
      if (result) {
        ...
        this.masterService.updateMasterGeo(item).subscribe((res: any) => {
          this.toastr.success(res.message);
        })
  ...
  assignParent() {
    this.masterService.getGeoHierarchy().subscribe((res: any) => {
